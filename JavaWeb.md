...
Http请求request协议包内部空间【背】：
  按照自下而上分成四层：
      1、请求行：主要存储url:请求地址、method:请求方式(POST/GET)
      2、请求头：请求参数信息【GET才会保存】,不同的键值对表示不同的含义
      3、空白行：没有任何内容，起到隔离的作用
      4、请求体：请求参数信息【POST才会保存】，就是发送给服务器的数据
...
常用的请求头：
  ...
  请求是post请求就用doPost()方法，请求是get请求就用doGet()方法
  怎么确定哪些是get请求或post请求？
    GET请求有哪些：
      1、form标签 method=get
      2、a 标签
...
​	POST请求：
​	  8、form标签 method=post，剩下的所有请求都可以是GET请求



Http响应response协议包内部空间【背】：
  按照自下而上分成四层：
      1、状态行：响应的协议+Http状态码+响应状态描述符
      2、响应头：Server(表示服务器的信息)+Content-type(指定浏览器采用对应编译器对响应体二进制数据进行解析)+Content-Length(响应体的长度)+Date(格林时间)
      3、空白行
      4、响应体：可能是被访问的静态资源文件内容，也可能是被访问的静态资源文件命令，还可能是被访问的动态资源文件结果，都是以二进制形式
...
- 请求方式：**Get，Post**，HEAD,DELETE,PUT,TRACT…
  - **get：请求能够携带的参数比较少，大小有限制，会在浏览器的URL地址栏显示数据内容，不安全，但高效**，只能提交字符串数据。GET请求最终的结果，会被浏览器缓存收纳。
  
  - **post：请求能够携带的参数没有限制，大小没有限制，不会在浏览器的URL地址栏显示数据内容，安全，但不高效**。可以提交任何类型的数据，文件上传必须使用POST提交。POST请求最终的结果不会被浏览器缓存
  
    二者的选择:
  
    ​	有敏感数据必须使用POST；传送的数据不是普通字符串，必须使用POST；传送的数据非常多，使用POST；请求是为了修改服务器端资源，使用POST；剩下的使用GET。GET请求多数情况下从服务器中读取资源，这个读取的资源在短时间内不会发生变化的，所以GET请求最终的结果被浏览器缓存起来了。POST请求是为了修改服务器端的资源，而每一次修改的结果都是不同的，最终结果没有必要被缓存。
...
404和500是HTTP协议状态码，是W3C制定的，所有浏览器和服务器都必须遵守
...
首次访问会创建一个servlet.class文件，之后访问只需要读取这个文件即可
...
1. 一个Servlet可以指定一个映射路径
   ...
   ```xml
       <servlet-mapping>
           <servlet-name>hello</servlet-name>
           <url-pattern>/hello</url-pattern>		映射地址记得加"/"
       </servlet-mapping>
   ```
...
3. 一个Servlet可以指定通用映射路径
   ...
   ```xml
       <servlet-mapping>
           <servlet-name>hello</servlet-name>
           <url-pattern>/hello/*</url-pattern>		//只要/hello开头的地址都转过去
       </servlet-mapping>
   ```
...
web容器在启动的时候，它会为**每个web程序都创建一个对应的ServletContext对象，它代表了当前的web应用；**

#### 1、共享数据

我在这个Servlet中保存的数据，可以在另外一个servlet中拿到(需要用到ServletContext)；

可以把一个Servlet中的数据存到ServletContext这个容器中，然后另一个Servlet可以获取来使用
...
public class GetServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        resp.setCharacterEncoding("utf-8");	//设置字符否则乱码
...
protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
    ...
    context.getRequestDispatcher("/gp").forward(req,resp);	//将上面两行合并
...
public class ServletDemo05 extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        InputStream is = this.getServletContext().getResourceAsStream("/WEB-INF/classes/com/kuang/servlet/aa.properties");	//这个位置是在母模板的target位置处!!!
        //把properties当作一个流读进来
...
web服务器接收到客户端的http请求，针对这个请求，分别创建一个**代表请求的HttpServletRequest对象，代表响应的一个HttpServletResponse**；
...
2、下载文件(要获取下载文件的路径、下载的文件名是啥、设置想办法让浏览器能够支持下载我们需要的东西)
...
protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
    ...
    /*
        ...
        resp.setStatus(302);
         */
    resp.sendRedirect("/r/img");//重定向,r是module的名字(更高一级的路径)
}
```
...
JSP声明：会被编译到 JSP生成Java的类中！其他的，就会被生成到_jspService方法中！
...
<error-page>
    ...
    <location>/xxx/xxx.jsp</location>		//需要跳转到jsp页面
</error-page>
```
...
```jsp
<%@page args.... %>
<%@include file=""%>		网站的头部脚部一般都是一样的公共页面
...
<%
//进行查找:	
	pageContext.findAttribute("name1");			//或pageContext.getAttribute("name1");
	request.findAttribute("name2");
    session.findAttribute("name3"); 
    application.findAttribute("name4");
%>
<%
	//使用EL表达式输出值:
	<h3>${name1}</h3>
...
<jsp:forward page="/jsptag2.jsp">
    <jsp:param name="name" value="kuangshen"></jsp:param>	<!--转发时携带这些参数-->
...
<c:if test="${param.username=='admin'}" var="isAdmin">		var接收返回值
    <c:out value="管理员欢迎您！"/>
</c:if>
...
# url-pattern编写方式和路径的总结:

一、路径的编写形式：
    1、<a href="/项目名/资源路径"></a>
    2、<form action="/项目名/资源路径"></form>
    3、转发：request.getRequestDispatcher("/资源路径").forward(request,response);
    4、重定向：response.sendRedirect("/项目名/资源路径");
    5、欢迎页面：
      <welcome-file-list>
    ...
    6、servlet路径：
      <servlet>
        ...
        <servlet-class>com.csu.login.xxx</servlet-class>
    </servlet>
    <servlet-mapping>
        <servlet-name>xxx</servlet-name>
    ...
    8、ServletContext application = config.getServletContext();
       application.getRealPath("/WEBINF/classes/db.properties");
